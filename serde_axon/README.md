...
```toml
serde_axon = { version = "1.0", default-features = false, features = ["alloc"] }
```
...
families. `tests/conformance.rs`, `tests/robustness.rs`, and
`tests/roundtrip.rs` add adversarial hardening and Serde round-trips. The full
suite passes, including `no_std + alloc` and rustdoc, and the crate is
