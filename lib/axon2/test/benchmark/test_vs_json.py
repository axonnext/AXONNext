...
import random
import time
from axon2.types import unicode_type
import gc

def random_string(n):
    text = ''.join(chr(ord('a')+random.randint(1,20)) for i in range(20))
    text = axon2.as_unicode(text)
    return text
...
class AXONvsJSON:
    ...
    def test_vs_json_1(self):
        ...
        axon_text = axon2.dumps(lst)
        gc.collect()
        gc.disable()
        t0 = time.time()
        v = axon2.loads(axon_text)
