...
#define CH_BETWEEN(x, a, b) (x >= a) && (x <= b)
    ...
    /* _PyLong_Format became internal-only (no public declaration) in CPython
       3.13, so calling it from an extension truncates the returned pointer and
       crashes. PyObject_Str gives the identical base-10 string for an int and
       is stable public API. */
    #define c_int_tostring(ob) PyObject_Str(ob)
