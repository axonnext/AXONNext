...
if sys.version_info.major == 3:
    ...
    int_mode = b'i'
_builder_dict = {'safe': SafeBuilder(), 'strict': StrictBuilder(), 'mixed': MixedBuilder()}
...
class Loader:
    ...
    def get_number(self):
        ...
        if ch == '.':
            ...
            if ch == 'T':
                ...
                val = self.sbuilder.create_datetime(self.da[0], self.da[1], self.da[2], self.ta[0], self.ta[1], self.ta[2], self.ta[3], tzinfo)
            else:
                val = self.sbuilder.create_date(self.da[0], self.da[1], self.da[2])
            return val
        elif ch == ':':
            self.pos = pos0
            v = self.get_time()
    ...
    def get_value(self, idn, idn0):
        """Depth-guarded entry point for value parsing.

        Bounds recursion depth so that pathologically nested input raises a
        normal `AxonError` instead of overflowing the C stack (the legacy
        `cdef` methods recurse on the C stack, where a `RecursionError` cannot
        fire). See `c_max_depth`.
        """
        self.c_depth += 1
        try:
            if self.c_depth > self.c_max_depth:
                errors.error(self, 'Maximum nesting depth exceeded')
            return self.c_get_value(idn, idn0)
        finally:
            self.c_depth -= 1

    def c_get_value(self, idn, idn0):
        ch = current_char(self)
        if ch == '#':
            self.skip_comments()
            ch = '\x00' if self.eof else current_char(self)
        if ch <= '9' and ch >= '0':
            val = self.get_number()
            return val
        if ch == '-':
